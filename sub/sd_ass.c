                        ...
                        struct sub_bitmaps *res)
{
    ...
    bool use_vs_video = !ctx->is_converted &&
        opts->ass_style_override ? opts->ass_vsfilter_video_compat : 1;
...
static void mangle_colors(struct sd *sd, struct sub_bitmaps *parts)
{
    ...
    // We probably shouldn't do this crap for unknown or non-YUV pixel formats.
    // Using a typical ASS script on RGB video shouldn't send the colors to
    // hell either.
        ...
        params.colorlevels != ctx->last_params.colorlevels)
    {
        ...
        mp_msg(MSGT_SUBREADER, MSGL_WARN, "[sd_ass] mangling colors like vsfilter: "
               "RGB -> %s %s -> %s %s -> RGB\n", mp_csp_names[csp],
